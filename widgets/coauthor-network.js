...
function render({ model, el }) {
  const dataUrl = resolveDataUrl(model.get("data_url") || "data/coauthors.json");
  const accentLight = model.get("accent_color") || "#8C1515";
  const accentDark = model.get("accent_color_dark") || accentLight;
  let accentColor = accentLight;   // switched by applyTheme() below
  ...
  function cnApplyTheme() {
    ...
    accentColor = dark ? accentDark : accentLight;
  ...
  let focusNode = null; // null = the PI (index 0)
  ...
  function init(data) {
    ...
    const sortedPapers = nodes.map(n => n.piPapers).sort((a, b) => b - a);
    const maxPiPapers = sortedPapers[1] || sortedPapers[0]; // use 2nd highest
    ...
    for (let i = 0; i < N; i++) {
      ...
      const t = Math.min(nodes[i].piPapers / maxPiPapers, 1);
      if (i === 0) {
        nodes[i].radius = 22;
        nodes[i].fontSize = 16;
      } else {
    ...
    function draw() {
      ...
      for (const i of drawOrder) {
        ...
        const t = Math.min(n.piPapers / maxPiPapers, 1);
    ...
    canvas.addEventListener("pointermove", (e) => {
      ...
      if (hit >= 0) {
        ...
        infoBox.innerHTML = `<b>${n.name}</b> · ${n.piPapers} paper${n.piPapers > 1 ? "s" : ""} · ${connections} co-authors`;
